...
# print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!sys.path ( ocr2_2 의 Lib 의 site-packages 추가후 ( 부모디렉토리 ) : {sys.path}')
...
#로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py 어디서 임포트 해오는 거임?? FakeTextDataGenerator : {FakeTextDataGenerator}')
#print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py os.getcwd()1 : {os.getcwd()}')
#로그용 주석처리_print(f'=============================run.py FakeTextDataGenerator 임포트 완료 FakeTextDataGenerator1 : {FakeTextDataGenerator}')
...
from custom_trdg.data_generator import FakeTextDataGenerator
from trdg.data_generator import FakeTextDataGenerator
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=os.path.join(os.path.split(os.path.realpath(__file__))[0], "images2/black_background"),
    )
    parser.add_argument(
        "-ca",
        "--case",
...
def main():
    ...
    #args.input_file = r'C:/Users/TAMSystech/yjh/ipynb/TextRecognitionDataGenerator/trdg/dicts/전체언어텍스트파일/th_failed.txt'
    ...
    # skew_angles = list(range(min_skew, max_skew + step, step))
    # skew_angles = list(range(min_skew, args.skew_angle, step))
    #skew_angles = np.arange(min_skew, max_skew + step, step).tolist()
    skew_angles = np.round(np.arange(min_skew, max_skew + step, step).tolist(), 2)
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py skew_angles : {skew_angles}')

    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py args.random_skew : {args.random_skew}')
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py args.skew_angle : {args.skew_angle}')
    ...
    # skew_angles을 동적으로 생성
    distorsions = list(range(min_distorsion, max_distorsion + step, step))
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsions : {distorsions}')

    min_distorsion_orientation = 0
    max_distorsion_orientation = 2
    max_distorsion_orientation = 1
    max_distorsion_orientation = 0
    step = 1

    # distorsion_orientations = [0, 1, 2, 3]
    # distorsion_orientations을 동적으로 생성
    distorsion_orientations = list(range(min_distorsion_orientation, max_distorsion_orientation + step, step))
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion_orientations : {distorsion_orientations}')
    ...
    # orientations = [0, 1]
    # orientations을 동적으로 생성
    orientations = list(range(min_orientation, max_orientation + step, step))
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py orientations : {orientations}')
    ...
    #                os.path.isdir(os.path.join(base_image_dir, d))]

    # base_image_dir = r"C:\Users\TAMSystech\yjh\ipynb\TextRecognitionDataGenerator\trdg\images2\black_background"

    image_dirs = []
    # os.walk를 사용하여 디렉토리 반복
    for dirpath, dirnames, filenames in os.walk(base_image_dir):
        ...
        for dirname in dirnames:
            ...
            image_dirs.append(os.path.join(dirpath, dirname))  # 디렉토리 경로를 리스트에 추가

    #print(f'image_dirs : {image_dirs}')
    ...
    # alignments = [0, 1, 2]
    # alignments을 동적으로 생성
    # alignments = list(range(min_alignment, max_alignment + step, step))
    alignments = np.arange(min_alignment, max_alignment + step, step).tolist()
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py alignments : {alignments}')
    ...
    # min_background = [0, 1, 2, 3]
    # backgrounds을 동적으로 생성
    # blurs = list(range(min_blur, max_blur + step, step))
    #blurs = np.arange(min_blur, max_blur + step, step).tolist()
    #blurs = np.arange(min_blur, max_blur + step, step).tolist()
    blurs = np.round(np.arange(min_blur, max_blur + step, step).tolist(), 2)

    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py skew_angles : {skew_angles}')
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py blurs : {blurs}')

    # random_blur

    # print(
    #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! fonts 블라블라 : {[fonts[rnd.randrange(0, len(fonts))] for _ in range(0, string_count)]}')
    # args.margins = 2
    ...
    print(
        f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py FakeTextDataGenerator.generate_from_tuple 함수 호출 전 os.getcwd() : {os.getcwd()}')
    ...
    # print(
    #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py FakeTextDataGenerator.generate_from_tuple 함수 호출 FakeTextDataGenerator : {FakeTextDataGenerator }')
    #
    # print(
    #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py FakeTextDataGenerator.generate_from_tuple 함수 호출 generate_from_tuple : {FakeTextDataGenerator.generate_from_tuple}')
    # 클래스 객체 생성
    generator_instance = FakeTextDataGenerator()
    # print(
    ...
    # output_dir = fr'{output_dir}\skew_angles'
    ...
    for skew_angle in skew_angles:
        args.random_skew = False
        #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py skew_angle : {skew_angle}')

        # args.skew_angle = skew_angle

        #args.skew_angle = rnd.randint(0 - skew_angle, skew_angle)
        #args.skew_angle = rnd.uniform(min_skew, max_skew)
        #args.skew_angle = round(rnd.uniform(min_skew, max_skew), 2)
        args.skew_angle = skew_angle
        #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py args.skew_angle : {args.skew_angle}')
        ...
        #output_dir = fr'{output_dir}\skew_angle({args.skew_angle})'
        #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py  args.skew_angle_skew_angle 추가후 output_dir : {output_dir}')

        #output_dir = os.path.join(output_dir, 'distorsion')

        for distorsion in distorsions:
            args.distorsion = distorsion
            #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion : {distorsion}')
            #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsions : {distorsions}')

            for distorsion_orientation in distorsion_orientations:
                args.distorsion_orientation = distorsion_orientation
                #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion_orientation : {distorsion_orientation}')

                for orientation in orientations:
                    #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py orientation : {orientation}')
                    args.orientation = orientation

                    for background in backgrounds:
                        #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py background : {background}')
                        args.background = background



                        for image_dir in image_dirs:
                            #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py image_dir : {image_dir}')
                            args.image_dir = image_dir
                            rgb = os.path.basename(image_dir)
                            #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py rgb : {rgb}')

                            for alignment in alignments:
                                #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py alignment : {alignment}')
                                args.alignment = alignment

                                for blur in blurs:
                                    #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py blur : {blur}')
                                    args.blur = blur
                                    ...
                                    #output_dir = os.path.join(output_dir, 'distorsion')
                                    #output_dir = fr'{output_dir}\skew_angle({args.skew_angle})'
                                    #output_dir = fr'{output_dir}\distorsion({args.distorsion})'
                                    # args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})'
                                    # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py   args.distorsion_distorsion 추가후 output_dir : {output_dir}')

                                    #for font in fonts:
                                    for font in fonts:
                                        p = Pool(args.thread_count)
                                        print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py args.thread_count : {args.thread_count}')
                                        #print('generate_from_tuple 병렬처리 =============================================================================')
                                        #print('generate_from_tuple 병렬처리 =============================================================================')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py args.random_skew : {args.random_skew}')
                                        #
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py blurs : {blurs}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py skew_angles : {skew_angles}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py skew_angle : {skew_angle}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsions : {distorsions}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion : {distorsion}')
                                        # print(
                                        #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion_orientations : {distorsion_orientations}')
                                        # print(
                                        #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py distorsion_orientation : {distorsion_orientation}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py orientations : {orientations}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py orientation : {orientation}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py backgrounds : {backgrounds}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py background : {background}')
                                        # #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py image_dirs : {image_dirs}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py image_dir : {image_dir}')
                                        #
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py alignments : {alignments}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py alignment : {alignment}')
                                        #
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py blurs : {blurs}')
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py blur : {blur}')
                                        # print('=============================================================================')
                                        #
                                        #
                                        # print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! p : {p}')
                                        ...
                                        #args.output_dir = fr'{output_dir}\{args.skew_angle}\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\{args.skew_angle}\{relative_path}'
                                        # skew_angle
                                        # distortion
                                        # distorsion_orientation
                                        ...
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\resize_orientation({args.orientation})\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\distorsion_orientation({args.distorsion_orientation})\resize_orientation({args.orientation})\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\distorsion_orientation({args.distorsion_orientation})\resize_orientation({args.orientation})\background({args.background})\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\distorsion_orientation({args.distorsion_orientation})\resize_orientation({args.orientation})\background({args.background})\alignment({args.alignment})\{relative_path}'
                                        #args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\distorsion_orientation({args.distorsion_orientation})\resize_orientation({args.orientation})\background({args.background})\alignment({args.alignment})\blur({args.blur})\{relative_path}'
                                        args.output_dir = fr'{output_dir}\skew_angle({args.skew_angle})\distorsion({args.distorsion})\distorsion_orientation({args.distorsion_orientation})\resize_orientation({args.orientation})\background({args.background})\background_rgb({rgb})\alignment({args.alignment})\blur({args.blur})\{relative_path}'

                                        # print(
                                        #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! run.py args.output_dir : {args.output_dir}')
                                        #args.output_dir = fr'{output_dir}\{relative_path}'

                                        #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py 최종 args.output_dir : {args.output_dir}')
                                        ...
                                        #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py  args.output_dir : {args.output_dir}')
                                        # print(
                                        #     f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py fonts : {[fonts[fonts.index(font)] for _ in range(0, string_count)]}')
                                        # 디렉토리가 없으면 생성
                                        if not os.path.exists(args.output_dir):
                                            os.makedirs(args.output_dir)
                                        #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py  args.output_dir : {args.output_dir}')
                                        #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py  args.distorsion_orientation : {args.distorsion_orientation}')
                                        ...
                                        p.terminate()
                                        #args.name_format = 2
                                        #로그용 주석처리_print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py  args.name_format : {args.name_format}')
                                        ...
                                        if args.name_format == 3:
                                            ...
                                            #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!run.py  out_dir 변경 ( labels 폴더에 labels.txt 생성 ) : {out_dir}')
                                            ...
                                            #         #file_name = strings[i] + "_" + str(i) + "." + args.extension
                                            #         file_name = f'image-{i:09d}.{args.extension}'
                                            #
                                            #         label = strings[i]
                                            #
                                            ...
                                            #         #로그용_주석처리_print(f' run.py label.txt 저장 완료 label : {label}')
